...
export interface ProgramClass {
  ...
  MstProgramAge?: any;
  MstProgramType?: ProgramType; // ⬅️ ditambahkan
...
const DataTableCard: React.FC = () => {
  ...
  const [dataProgramType, setDataProgramType] = useState<ProgramType[]>([]);
  ...
  const fetchDataProgramType = async () => {
    const token = localStorage.getItem("token");
    ...
    try {
      const response = await axios.get(
        `${API_URL}/admin/v1/program-management/program-type/for-select`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (response.data.success) {
        // Filter hanya yang aktif (mpt_status = true)
        const programTypes: ProgramType[] = response.data.data
          .filter((item: ProgramType) => item.mpt_status)
          .map((item: ProgramType) => ({
            mpt_id: item.mpt_id,
            mpt_name: item.mpt_name,
            mpt_status: item.mpt_status,
            created_at: item.created_at,
            updated_at: item.updated_at,
          }));

        setDataProgramType(programTypes);

      } else {
        showError("Failed to fetch program types");
      }
    } catch (error) {
      console.error(error);
      showError("Error fetching program types from server");
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (values.mpt?.id) {
        formData.append("mpt_id", values.mpt.id);
      }      
  ...
  const handleCreate = async () => {
    try {
      ...
      // Tambahkan Program Type
      formData.append("mpt_id", values.mpt_id?.id || "");      
              ...
              <Col span={6}>
                ...
                >
                  <Select placeholder="Select program type">
                    {dataProgramType.map((type) => (
                      <Select.Option key={type.mpt_id} value={type.mpt_id}>
                        {type.mpt_name}
                      </Select.Option>
                    ))}
                  </Select>
