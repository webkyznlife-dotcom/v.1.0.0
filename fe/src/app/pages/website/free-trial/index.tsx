...
const FreeTrialWebsite: React.FC = () => {
  ...
  const fetchDataProgram = async () => {
    try {
      const response = await axios.get(`${API_URL}/user/v1/program/for-select`); 
    ...
    <div style={styles.container}>
      <Container style={{ padding: 0, border: 0, paddingBottom: "50px" }}>
        ...
        <div ref={sectionRef} className="mt-16 mb-12 px-6 pt-0 w-[50%] mx-auto">
          ...
          >
            Join our community with no commitment required. Enjoy access to
            over 100 weekly classes, guided by expert certified coaches, all
            within a welcoming and family-friendly environment.
          </motion.p>
        </div>
