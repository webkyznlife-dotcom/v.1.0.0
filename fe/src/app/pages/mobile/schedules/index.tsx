...
import { MobileHourList } from "../schedules/components/schedules-listview/MobileHourList";
import { CourtHourList } from "../schedules/components/schedules-listview/CourtHourList";
...
interface ScheduleSlot {
  program: string;
  age: string;
  date: string;       // tanggal, misal "2025-10-02"
  time: string;       // jam, misal "08:00"
  instructor: string;
}
...
const SchedulesMobile: React.FC = () => {
  ...
  const toggleCourt = (court: string) =>
    setActiveCourt(activeCourt === court ? null : court);  
...
const fetchDataSchedules = async (
  ...
  try {
    const response = await axios.post(`${API_URL}/user/v1/schedules/formatted-mobile`, {
      selectedAge: ageParam ?? selectedAge,
      selectedBranch: branchParam ?? selectedBranch,
      selectedCourt: courtParam ?? selectedCourt,
      pickDate: dateParam ?? pickDate,
    });

    if (response.data.success) {
      const apiRows: ScheduleRow[] = response.data.data;

      // mapping agar sesuai ScheduleRowOutput
      const scheduleRowsOutput: ScheduleRowOutput[] = apiRows.map((row) => ({
        room: row.room,
        schedules: row.schedules.map((s) =>
          s
            ? {
                ...
                date: s.date, // wajib ada
              }
            : null
        ),
      }));

      setSchedules(scheduleRowsOutput); // sekarang aman
    } else {
    ...
    <div style={styles.container}>
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 px-4 sm:px-6 lg:px-10 mb-6">
            ...
            <div className="w-full">
              ...
              <DatePicker
                value={pickDate ? dayjs(pickDate, "YYYY-MM-DD") : null}
                onChange={(date, dateString) => {
                  if (typeof dateString === "string") {
                    setPickDate(dateString); // aman, hanya string
                  }
                }}
                className="w-full"
                format="YYYY-MM-DD"
      ...
      {showScheduleList && (
        <div className="px-4 sm:px-6 lg:px-10 space-y-3">
          {schedules.map((court) => {
            const hasSchedules = court.schedules.some((s) => s !== null);

            return (
              <div
                key={court.room}
                className={`rounded-lg overflow-hidden shadow-sm transition-colors ${
                  hasSchedules ? "bg-green-50" : "bg-white"
                }`}
              >
                <button
                  className="w-full text-left px-4 py-3 flex justify-between items-center hover:bg-gray-100 transition-colors"
                  onClick={() => toggleCourt(court.room)}
                >
                  <div>
                    <div className="font-semibold text-gray-800">{court.room}</div>
                    <div className={`text-sm ${hasSchedules ? "text-green-800" : "text-gray-500"}`}>
                      {hasSchedules
                        ? `${court.schedules.filter((s) => s !== null).length} Programs`
                        : "- (kosong)"}
                    </div>
                  </div>
                  <div
                    className={`transform transition-transform ${
                      activeCourt === court.room ? "rotate-180" : ""
                    }`}
                  >
                    <svg
                ...
                {activeCourt === court.room && hasSchedules && (
                  <div className="mt-2 border-t border-gray-200">
                    <CourtHourList rows={[court]} /> {/* hanya court ini */}
                  </div>
                )}
              </div>
            );
